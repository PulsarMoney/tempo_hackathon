...
export function PriceLineOverlay({
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="pointer-events-none absolute inset-y-0 left-0 right-[62px] h-full w-auto">
      <defs>
        <filter id="glow">
      ...
      <polyline points={polyline} fill="none" stroke="#34d399" strokeWidth="2" filter="url(#glow)" />
      <circle cx={currentX} cy={currentY} r="5" fill="#4ade80" className="animate-pulse" filter="url(#glow)" />
    </svg>
  );
}
